...
class LerobotDataset(LeRobotSingleDataset, BaseImageDataset):
    def __init__(
        ...
        if self.lang_emb is not None:
            assert language_modality_keys, (
                "Language modality keys should not be empty if lang_emb is defined"
            )
    ...
    def _get_lang_embeddings(self):
        ...
        for ep_batch in tqdm(
            np.array_split(
                self.trajectory_ids, int(math.ceil(len(self.trajectory_ids) / 64))
            )
        ):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key, lerobot_keys in self.rgb_keys.items():
            assert len(lerobot_keys) == 1, (
                f"multiple lerobot keys for {key} not supported"
            )
            ...
            obs_dict[key] = (
                np.moveaxis(data[lerobot_key][T_slice], -1, 1).astype(np.float32)
                / 255.0
            )
            # T,C,H,W
        ...
        if self.lang_emb is not None:
            ...
            obs_dict[LANG_EMB_KEY] = np.tile(lang_emb, (self.n_obs_steps, 1)).astype(
                np.float32
            )
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        assert not self.abs_action, (
            "normalizer for abs_action is not supported in LerobotDataset"
        )
...
class LerobotCotrainingDataset(LeRobotMixtureDataset, BaseImageDataset):
    def __init__(
        ...
        if dataset_soup is not None:
            ...
            dataset_soup_list = [
                {"path": ds_path, "filter_key": None} for ds_path in dataset_paths
            ]
        ...
        for i in range(len(dataset_soup_list)):
            ...
            dataset_soup_list[i]["ds_weight"] = dataset_soup_list[i].get(
                "ds_weight", None
            )
        ...
        self.abs_action = abs_action
        assert not self.abs_action, (
            "abs_action is not supported in LerobotCotrainingDataset"
        )
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for key, lerobot_keys in self.lowdim_keys.items():
            ...
            for k, v in stat.items():
                stat[k] = np.asarray(v, dtype=np.float32)
