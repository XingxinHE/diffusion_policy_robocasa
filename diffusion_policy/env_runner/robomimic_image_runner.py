...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def __init__(
        ...
        self.env_kwargs = (
            OmegaConf.to_container(env_kwargs) if env_kwargs is not None else {}
        )
        ...
        # RoboCasa currently exposes Gym/Gymnasium semantics that do not batch
        # cleanly with the vendored async vector env, so prefer the sync path.
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        success_rate = (
            sum([np.max(all_rewards[i]) > 0 for i in range(n_inits)]) / n_inits
        )
